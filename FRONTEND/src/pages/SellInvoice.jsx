...
export default function SellInvoice() {
  ...
  const [activeIndex, setActiveIndex] = useState(null); // which row’s search is active
  ...
  const handleChange = (index, field, value) => {
    ...
    const price = parseFloat(newItems[index].sellPrice) || 0;
  ...
  const fetchProduct = async (query, index) => {
    ...
    try {
      const response = await fetch(`${URL}/search_product?search=${query}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (data.mess === "success") {
        ...
        setActiveIndex(index); // which row is active
      } else {
        setSearchResults([]);
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-8xl p-5 overflow-y-auto">
      <h1 className="text-xl font-bold mb-6">Sell Invoice</h1>
      <form
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative mb-2">
          ...
          <input
            ...
            onChange={VendorhandleChange}
            required={true}
            className="col-span-1 md:col-span-3 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:border-indigo-500 "
          />
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          fetchProduct(e.target.value, idx); // call search API
                        }}
                        onFocus={() => setActiveIndex(idx)} // track focus
                        onBlur={() =>
                      ...
                      {activeIndex === idx && searchResults.length > 0 && (
                        <ul className="absolute z-20 bg-white border border-gray-300 w-full max-h-40 overflow-y-auto rounded-md shadow-md">
                          {searchResults.map((item, i) => (
                            <li
                              key={i}
                              onClick={() => {
                                const newItems = [...items];
                                newItems[idx].productName = item.productName;
                                newItems[idx].purchasePrice =
                                  item.purchesPrice || 0;
                                newItems[idx].units = item.units || 0;
                                setItems(newItems);
                                setSearchResults([]);
                              }}
                              className="px-3 py-1 hover:bg-indigo-100 cursor-pointer text-sm"
                            >
                              {item.productName} — ₹{item.purchesPrice}
                            </li>
                          ))}
                        </ul>
                      )}
        ...
        <div className="mt-6 p-4  space-y-3">
          ...
          <div className="flex justify-end text-lg">
            <button
              type="submit"
              className="relative cursor-pointer opacity-90 hover:opacity-100 transition-opacity p-[2px] bg-black rounded-[10px] bg-gradient-to-t from-[#4F39F6] to-[#4F39F6] active:scale-95"
            >
              <span className="w-full h-full flex items-center gap-2 px-6 py-1 bg-[#4F39F6] text-white rounded-[10px] bg-gradient-to-t from-[#4F39F6] to-[#4F39F6]">
                Save Invoice
              </span>
            </button>
          </div>
